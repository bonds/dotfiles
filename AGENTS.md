...
- **GitHub pushes from a Hermes agent session can fail auth silently.** Secretive's keys live in the SSH agent only when a TouchID-approved session holds them; a fresh agent session has **no identities** (`ssh-add -l` → "The agent has no identities"), `ssh github.com:22` may time out, and `ssh.github.com:443` connects but Secretive "refused operation" (can't do the TouchID prompt non-interactively). The `gh` CLI token is often read-only and 403s on push. If a push fails with `Permission denied (publickey)`/403, ask the user to run it (e.g. `cd ~/Documents/undated/repos/<repo> && git push origin main`) — don't assume a broken remote.
