...
$text" | env OLLAMA_HOST=192.168.4.43:11434 timeout 15 ollama run qwen2.5:0.5b 2>/dev/null | string collect)
    ...
    if test (count $m) -ge 5
        ...
        command curl -sL --max-time 8 "$raw_url" 2>/dev/null | head -c 50000 | __print_lines
        return
    end
    ...
    if test (count $m2) -ge 4
        if command --query gh
            ...
            if test -n "$release_notes"
                echo "$release_notes" | __print_lines
                return
            end
        end
    end
    ...
    if command --query lynx
        echo "$content" | command lynx -stdin -dump -nolist 2>/dev/null | head -c 40000 | __print_lines
    else if command --query w3m
        echo "$content" | command w3m -dump -T text/html 2>/dev/null | head -c 40000 | __print_lines
