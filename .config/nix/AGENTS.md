...
- **The neocode overlay on accismus (`modules/neocode-overlay.nix`) is intentionally pinned.** Community macOS SwiftUI client for OpenCode, built from the `bonds/NeoCode` fork at tag `v0.8.1-nix`. The fork carries custom patches: smaller default window (1100×700, min 960×640) for 13" MacBook screens + `/run/current-system/sw/bin` and `~/.nix-profile/bin` in NeoCode's `enhancedPATH` for automatic opencode CLI discovery. The overlay fetches the prebuilt `.dmg`, extracts via `7zz`, strips `SUFeedURL`/`SUEnableAutomaticUpdates` from Info.plist to disable Sparkle auto-update (`nr --update` is the only path), and re-signs ad-hoc. `nr --update` queries the fork's releases. To update: rebase fork on upstream, `xcodebuild build`, `hdiutil create`, `gh release create`, then `nr --update`.
