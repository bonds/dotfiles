...
- **Local-only secrets on the server must have a `warn_missing` check.** If a service reads a secret file that lives outside the repo (e.g. `/etc/ddns-token`, `/etc/email-pass`, `/dragon/docker/eufy-security-ws/.env`), add a corresponding `warn_missing` check in `system.activationScripts.checkSecrets` in `hosts/sophrosyne/configuration.nix`. This prints a clear warning at activation time telling the admin what the secret is for and where to find it (e.g. Bitwarden).
...
- **Sophrosyne's dotfiles repo is a normal clone at `~`** (work tree is `$HOME`, git dir is `~/.git`). `core.bare` must stay `false`. Do NOT set `core.bare = true` — it breaks the working tree. If pushes fail with "unstaged changes", reset with `ssh sophrosyne.local "cd ~ && git fetch origin && git reset --hard origin/main"` (laptop is source of truth). The `authorized_keys` symlink in the repo points to the macOS path (`/Users/scott/...`); the NixOS activation script in `hosts/sophrosyne/configuration.nix` overwrites it with the correct Linux path on each rebuild.
