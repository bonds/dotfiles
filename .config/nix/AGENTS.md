...
- **The neocode overlay on accismus (`modules/neocode-overlay.nix`) is intentionally pinned.** Community macOS SwiftUI client for OpenCode. The overlay fetches the prebuilt `.dmg` from `watzon/NeoCode/releases`, extracts via `7zz`, and strips `SUFeedURL`/`SUEnableAutomaticUpdates` from Info.plist to disable Sparkle auto-update (nr --update is the only path). Uses `dontFixup = true`. Added to `nr --update`'s package loop alongside the other pinned overlays.
