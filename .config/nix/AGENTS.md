...
- **The neocode package on accismus is consumed via a flake input** (`github:bonds/NeoCode`). The fork's `flake.nix` exposes `packages.aarch64-darwin.default` that fetches the prebuilt `.dmg`, extracts via `7zz`, strips Sparkle auto-update keys, and re-signs ad-hoc. The DMG hash lives in NeoCode's `flake.nix`, next to the source. To update: rebase fork on upstream, `xcodebuild build`, `hdiutil create`, `gh release upload --clobber`, prefetch the new DMG hash, update hash in NeoCode's `flake.nix`, commit NeoCode. Then `nr --update` in the dotfiles repo bumps the `neocode` flake input to the new commit.
